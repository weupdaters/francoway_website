...
@section('content')
  ...
  <div class="main-content-container overflow-hidden">
    ...
    <div class="d-flex align-items-center gap-3 mb-4">
      ...
      <div>
        <h3 class="mb-1 fw-bold text-dark fs-24">Edit Lesson</h3>
        <p class="text-muted mb-0 fs-14">Edit lesson details to keep your content up-to-date.</p>
      </div>
    </div>
      ...
      <div class="row">
        ...
        <div class="col-lg-8">
          <div class="card bg-white p-4 rounded-20 border border-white mb-4">
            ...
            {{-- Course (Read-only reference) --}}
            <div class="mb-4">
              <label class="form-label text-primary-indigo fw-bold fs-11 tracking-wider mb-2">COURSE</label>
              <div class="input-icon-group">
                <i class="bi bi-journal-bookmark"></i>
                <input type="text" class="form-control" value="{{ $lesson->course->title }}" readonly style="background-color: #f8fafc !important;">
              </div>
            </div>
            ...
            <div class="mb-4">
              ...
              <div class="input-icon-group">
                ...
                <input type="text" class="form-control" name="title" id="lessonTitleInput" value="{{ $lesson->title }}" placeholder="Enter lesson title" maxlength="150" required>
              ...
              <div class="input-icon-group">
                ...
                <select name="lesson_type" id="lessonType" class="form-select" required>
                  ...
                  @foreach(['video','pdf','image','audio','summary'] as $type)
                    <option value="{{ $type }}" {{ $lesson->lesson_type === $type ? 'selected' : '' }}>
                      {{ ucfirst($type) }}
                    </option>
                  @endforeach
                </select>
            ...
            {{-- Dynamic Inputs based on Lesson Type (Inline for clean layout) --}}
            <div class="dynamic-inputs-container">
              
              {{-- VIDEO --}}
              <div class="lesson-input {{ $lesson->lesson_type !== 'video' ? 'd-none' : '' }} mb-4 p-3 bg-light rounded-12 border" id="videoInput">
                <label class="form-label fw-semibold mb-2"><i class="bi bi-camera-video text-primary me-1"></i>Video</label>

                {{-- Existing Video Thumbnail --}}
                @if ($lesson->lesson_type === 'video' && $lesson->video_file)
                  <div class="mb-3" id="existingVideoWrap">
                    ...
                    <small class="text-muted d-block mt-1">{{ basename($lesson->video_file) }}</small>
                  </div>
                @endif

                {{-- New Upload Preview --}}
                <div id="videoPreviewWrap" class="d-none mb-2">
                  <p class="text-muted small mb-1">New Upload Preview:</p>
                  <div style="position:relative;border-radius:10px;overflow:hidden;background:#071530;">
                    <video id="videoPreviewEl" controls style="width:100%;max-height:180px;display:block;"></video>
                    <button type="button" id="clearVideoBtn"
                      style="position:absolute;top:8px;right:8px;background:rgba(0,0,0,0.6);border:none;border-radius:50%;width:28px;height:28px;color:#fff;font-size:14px;cursor:pointer;">✕</button>
                  </div>
                  <small class="text-muted mt-1 d-block" id="videoFileName"></small>
                </div>
                <label class="form-label small text-muted mb-1">Replace Video:</label>
                <input type="file" name="video_file" id="videoFileInput" class="form-control" accept="video/mp4,video/mov,video/avi">
              </div>

              {{-- PDF --}}
              <div class="lesson-input {{ $lesson->lesson_type !== 'pdf' ? 'd-none' : '' }} mb-4 p-3 bg-light rounded-12 border" id="pdfInput">
                <label class="form-label fw-semibold mb-2"><i class="bi bi-file-earmark-pdf text-danger me-1"></i>PDF</label>

                @if ($lesson->lesson_type === 'pdf' && $lesson->pdf_file)
                  <div class="mb-3">
                    <p class="text-muted small mb-1">Current PDF:</p>
                    <div class="d-flex align-items-center gap-2 p-2 rounded" style="background:#fff0f0;border:1px dashed #e74c3c;">
                      ...
                      <div>
                        <div class="fw-semibold small">{{ basename($lesson->pdf_file) }}</div>
                        <a href="{{ asset('storage/' . $lesson->pdf_file) }}" target="_blank" class="text-primary small">View PDF &rarr;</a>
                      </div>
                    </div>
                  </div>
                @endif
                ...
                <label class="form-label small text-muted mb-1">Replace PDF:</label>
                <input type="file" name="pdf_file" id="pdfFileInput" class="form-control" accept="application/pdf">
              </div>

              {{-- IMAGE --}}
              <div class="lesson-input {{ $lesson->lesson_type !== 'image' ? 'd-none' : '' }} mb-4 p-3 bg-light rounded-12 border" id="imageInput">
                <label class="form-label fw-semibold mb-2"><i class="bi bi-image text-success me-1"></i>Image</label>

                @if ($lesson->lesson_type === 'image' && $lesson->image_file)
                  <div class="mb-3" id="existingImageWrap">
                    ...
                    <small class="text-muted d-block mt-1">{{ basename($lesson->image_file) }}</small>
                  </div>
                @endif
                ...
                <div id="imagePreviewWrap" class="d-none mb-2">
                  ...
                  <div style="position:relative;display:inline-block;width:100%;">
                    <img id="imagePreviewEl" src="" alt="Preview"
                      style="width:100%;max-height:180px;object-fit:cover;border-radius:10px;border:1px solid #e0e0e0;">
              ...
              {{-- AUDIO --}}
              <div class="lesson-input {{ $lesson->lesson_type !== 'audio' ? 'd-none' : '' }} mb-4 p-3 bg-light rounded-12 border" id="audioInput">
                <label class="form-label fw-semibold mb-2"><i class="bi bi-mic text-warning me-1"></i>Audio</label>

                @if ($lesson->lesson_type === 'audio' && $lesson->audio_file)
                  <div class="mb-3">
                    <p class="text-muted small mb-1">Current Audio:</p>
                    <audio controls style="width:100%;">
                      <source src="{{ asset('storage/' . $lesson->audio_file) }}">
                    </audio>
                    <small class="text-muted d-block mt-1">{{ basename($lesson->audio_file) }}</small>
                  </div>
                @endif

                <div id="audioPreviewWrap" class="d-none mb-2">
                  <p class="text-muted small mb-1">New Upload:</p>
                  <audio id="audioPreviewEl" controls style="width:100%;"></audio>
                  <small class="text-muted mt-1 d-block" id="audioFileName"></small>
                </div>
                <label class="form-label small text-muted mb-1">Replace Audio:</label>
                <input type="file" name="audio_file" id="audioFileInput" class="form-control" accept="audio/mp3,audio/wav">
              </div>

              {{-- SUMMARY --}}
              <div class="lesson-input {{ $lesson->lesson_type !== 'summary' ? 'd-none' : '' }} mb-4 p-3 bg-light rounded-12 border" id="summaryInput">
                <label class="form-label fw-semibold mb-2"><i class="bi bi-file-text me-1"></i>Summary</label>
                <textarea name="summary" class="form-control" rows="4" placeholder="Enter summary here...">{{ $lesson->summary }}</textarea>
              </div>
  ...
  <script>
    $(document).ready(function() {
      ...
      const updateCharCount = () => {
        charCounter.text(`${titleInput.val().length} / 150`);
      };
      titleInput.on('input', updateCharCount);
      ...
      function toggleTypeInputs(selected) {
        ...
        if (selected === 'video') $('#videoInput').removeClass('d-none');
        if (selected === 'pdf') $('#pdfInput').removeClass('d-none');
        if (selected === 'image') $('#imageInput').removeClass('d-none');
        if (selected === 'audio') $('#audioInput').removeClass('d-none');
        if (selected === 'summary') $('#summaryInput').removeClass('d-none');
      }
      ...
      $('#imageFileInput').on('change', function() {
        ...
        // Hide the existing image to make way for preview
      ...
      $('#videoFileInput').on('change', function() {
        ...
        $('#existingVideoWrap').addClass('d-none');
      });
      $('#clearVideoBtn').on('click', function() {
        ...
        $('#existingVideoWrap').removeClass('d-none');
      });
