        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb align-items-center mb-0 lh-1">
                ...
                <li class="breadcrumb-item active">
                    <span class="text-secondary">Edit Prompt</span>
                </li>
            </ol>
        </nav>
    </div>
                ...
                <div class="card bg-white p-20 rounded-10 border border-white mb-4">
                    ...
                    <div class="mb-20">
                        ...
                        <select name="course_id" class="form-select @error('course_id') is-invalid @enderror">
                            ...
                            @foreach($courses as $course)
                                <option value="{{ $course->id }}" {{ old('course_id', $prompt->course_id) == $course->id ? 'selected' : '' }}>
                                    {{ $course->title }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-20">
                        ...
                        <select name="skill" class="form-select @error('skill') is-invalid @enderror" required>
                            ...
                            <option value="reading" {{ old('skill', $prompt->skill) === 'reading' ? 'selected' : '' }}>Reading</option>
                            <option value="listening" {{ old('skill', $prompt->skill) === 'listening' ? 'selected' : '' }}>Listening</option>
                            <option value="speaking" {{ old('skill', $prompt->skill) === 'speaking' ? 'selected' : '' }}>Speaking</option>
                            <option value="writing" {{ old('skill', $prompt->skill) === 'writing' ? 'selected' : '' }}>Writing</option>
                        </select>
                        @error('skill')
                            <div class="invalid-feedback text-danger mt-1">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-20">
                        ...
                        <textarea name="prompt_text"
                                  ...
                                  required>{{ old('prompt_text', $prompt->prompt_text) }}</textarea>
                        @error('prompt_text')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="card bg-white p-20 rounded-10 border border-white mb-4">
                    ...
                    <div class="mb-20">
                        ...
                        <select name="status" class="form-select">
                            <option value="1" {{ old('status', $prompt->status ? '1' : '0') == '1' ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ old('status', $prompt->status ? '1' : '0') == '0' ? 'selected' : '' }}>Inactive</option>
                        </select>
                    </div>
